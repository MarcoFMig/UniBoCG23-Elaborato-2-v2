...
/**
 * Gathers shader files and compiles all required shaders.
 */
void INIT_SHADER() {
...
/**
 * Initializes the VAO and builds
...
void INIT_VAO() {
	...
	for (int i = 0; i < nmeshes; i++) {
		...
		Model3D[i].ModelM = translate(Model3D[i].ModelM, vec3(0.0f, 0.0f, 12.0f));
		Model3D[i].ModelM = scale(Model3D[i].ModelM, vec3(0.01f, 0.01f,0.01f));
    float angleInDegrees = -90.0f; // Sostituisci con l'angolo desiderato
    float angleInRadians = glm::radians(angleInDegrees);
    glm::vec3 rotationAxis = glm::vec3(1.0f, 0.0f, 0.0f); // Asse di rotazione (Y-axis in questo esempio)
    Model3D[i].ModelM = glm::rotate(Model3D[i].ModelM, angleInRadians, rotationAxis);
...
/**
 * Inits lighting properties for every existing material and creates a white movable light
 */
